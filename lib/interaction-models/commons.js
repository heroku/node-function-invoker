...
async function marshallPayload(payload, contentType) {
    ...
    const marshalledPayload = await attempt(marshall, payload, 'error-client-marshall');
    return marshalledPayload;
}

async function unmarshallPayload(payload, contentType) {
    const unmarshall = unmarshaller(contentType);
    if (!unmarshall) {
        throw new RiffError('error-client-content-type-unsupported');
    }
    const unmarshalledPayload = await attempt(unmarshall, payload, 'error-client-unmarshall');
    return unmarshalledPayload;
}

async function buildMessage(message, { contentType, correlationId } = {}) {
    ...
    try {
        const marshalledOutput = await marshallPayload(payload, contentType);
...
async function attempt(fn, arg, type) {
    try {
        return await fn(arg);
    } catch (e) {
