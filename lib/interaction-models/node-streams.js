...
function nodeStreamsInteractionModel(fn, argumentTransformer, messageStream) {
    ...
    const toFunction = miss.through.obj(async function (chunk, enc, callback) {
        ...
        try {
            ...
            const unmarshalledPayload = await unmarshallPayload(payload, contentType);
    ...
    const fromFunction = miss.through.obj(async function (chunk, enc, callback) {
        const message = await buildMessage(chunk, {
